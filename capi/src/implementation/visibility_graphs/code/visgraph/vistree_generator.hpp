...
class VistreeGenerator {
  public:
    ...
    [[nodiscard]] std::vector<VisibleVertex> get_visible_vertices(const Coordinate &observer, bool half_scan = false) const;
    [[nodiscard]] std::vector<VisibleVertex> get_visible_vertices_from_candidate_segments(const Coordinate &observer, const std::vector<std::shared_ptr<LineSegment>> &candidate_segments, bool half_scan = false) const;
  ...
  private:
    ...
    static std::vector<Coordinate> all_vertices_for_line_segments(const std::vector<std::shared_ptr<LineSegment>>& line_segments);
    ...
    [[nodiscard]] bool is_vertex_visible(const OpenEdges &open_edges,
