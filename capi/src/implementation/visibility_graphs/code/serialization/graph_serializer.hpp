...
class GraphSerializer {
  ...
  private:
    ...
    static size_t deserialize_polygon_vertices_from_mmap(const mio::mmap_source& mmap, Graph& graph, uint64_t num_polygons, size_t offset);
    static size_t deserialize_adjacency_matrix_from_mmap(const mio::mmap_source& mmap, Graph& graph, size_t offset);
};
